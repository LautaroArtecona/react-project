...
export const DataContext = ({ children }) => {
  ...
  const searchImages = async () => {
    ...
    let URL = `https://api.unsplash.com/search/photos/?client_id=${API_KEY3}&query=${seachValue}&per_page=6&`;
  ...
  useEffect(() => {
    const searchImages = async () => {
      let URL = `https://api.unsplash.com/search/photos/?client_id=${API_KEY3}&query=${seachValue}&per_page=6&page=${page}`;
  ...
  useEffect(() => {
    if (!seachValue == "") {
      const fetchTags = async () => {
        setVariable(3);
        let URL = `https://api.unsplash.com/search/photos/?client_id=${API_KEY}&query=${seachValueTag}&per_page=6`;
        const response = await fetch(URL);
        const data = await response.json();
  ...
  useEffect(() => {
    //if(!valor==''){
    const fetchTags = async () => {
      let URL = `https://api.unsplash.com/search/photos/?client_id=${API_KEY}&query=${seachValueTag}&per_page=6&page=${pageTag}`;
      const response = await fetch(URL);
      const data = await response.json();
